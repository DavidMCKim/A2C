    ...
    def Download_Stable_Chrome_Version(self):
        try:
            ...
            for version in self.versions:
                try:
                    if version['platform'] == 'linux64':
                        self.download_link = version['url']
                        break
                except Exception as e:
                    print(e)
            # 드라이버를 저장할 파일 경로
            self.driver_filename = "chromedriver_linux64.zip"  # 저장할 파일명
            # self.driver_path = f'D:\\0_mckim\\wordprocess\\chrome_version'  # 실제 저장할 경로로 수정
            self.driver_path = os.getcwd() + '/crawling/chrome_version'
